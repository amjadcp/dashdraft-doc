...
/**
 * Netlify Function handler (API v2) for Serving and Tracking install.sh
 */
export default async (request: Request, context: Context) => {
  let scriptContent = "";
  
  try {
    // Resolve absolute path to the bundled shell script
    const filePath = path.join(process.cwd(), "scripts", "install.sh");
    scriptContent = fs.readFileSync(filePath, "utf8");
  } catch (err) {
    console.error("Failed to read scripts/install.sh:", err);
  ...
  // Determine platform based on User-Agent hint if possible
  const ua = (request.headers.get("user-agent") || "").toLowerCase();
  const platform = ua.includes("darwin") || ua.includes("mac") ? "macos" : "linux";

  // Dispatch GA4 Measurement Protocol event
  const trackingPromise = trackDownload("install_sh_download", platform, request);
  ...
  // Return the installer shell script content immediately
